...
class SnakeGame:
    ...
    def do_training(self):
        ...
        for game in range(1, self.args.NUM_TRAIN_ITER + 1):
            ...
            if game % NUM_TO_STAT == 0:
                print(
                    f"Played games: {game - NUM_TO_STAT} - {game}, "
                    f"Average points: {sum(self.points_results[-NUM_TO_STAT:]) / NUM_TO_STAT}, "
                    f"Max points: {max(self.points_results[-NUM_TO_STAT:])}, "
                    f"Min points: {min(self.points_results[-NUM_TO_STAT:])}"
                )
